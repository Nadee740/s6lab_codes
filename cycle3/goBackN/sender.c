...
#include <string.h>
 ...
 void sendWindowPackets ( int * sockfd , struct sockaddr_in addr , int packets [] , int windowStart
, int windowEnd )
 {
 char buffer [1024];
...
void main ()
 ...
 int rec = recvfrom ( sockfd , buffer , 1024 , 0, ( struct sockaddr *) & addr , &
addr_size ) ;
 if ( rec < 0)
  {
 ...
 printf (" Client : Sending packet %s \n", buffer );
 sendto ( sockfd , buffer , 1024 , 0, ( struct sockaddr *) & addr , sizeof (
addr )) ;
 }
 if( windowEnd = packetCount )
 flag =0;
